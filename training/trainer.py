...
def do_train(dataloaders, params: MinkLocParams, debug=False, visualize=False):
    ...
    if params.resume:
        # checkpoint = torch.load('params.pretrained_path')
        model_path = '../weights/' + params.resume_name + '_final.pth'
        checkpoint = torch.load(model_path)
        model.load_state_dict(checkpoint, strict=False)

        pickle_path = '../weights/' + params.resume_name + '_stats.pickle'
    ...
    # for epoch in tqdm.tqdm(range(1, 6)):
    for epoch in tqdm.tqdm(range(1, params.epochs + 1)):
        ...
        # if epoch % 5 == 0:
        #     final_model_path = model_pathname + '_' + str(epoch) + '.pth'
...
def export_eval_stats(file_name, prefix, eval_stats, dataset_name):
    ...
    with open(file_name, "a") as f:
        if dataset_name == 'USyd':
            ave_1p_recall = eval_stats[dataset_name]['ave_one_percent_recall']
            ave_1p_recall_l.append(ave_1p_recall)
            ave_recall = eval_stats[dataset_name]['ave_recall'][0]
            ...
            s += ", {:0.2f}, {:0.2f}".format(ave_1p_recall, ave_recall)
                ...
                ave_1p_recall = eval_stats['oxford']['ave_one_percent_recall']
                ave_1p_recall_l.append(ave_1p_recall)
                ave_recall = eval_stats['oxford']['ave_recall'][0]
                ave_recall_l.append(ave_recall)
                s += ", {:0.2f}, {:0.2f}".format(ave_1p_recall, ave_recall)
