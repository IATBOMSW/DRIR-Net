...
class Gate(nn.Module):
    def __init__(self, output_dim, add_batch_norm=True):
        ...
        self.gating_weights = nn.Parameter(
            torch.randn(output_dim, output_dim) * 1 / math.sqrt(output_dim))
        self.sigmoid = nn.Sigmoid()
        ...
        if add_batch_norm:
            ...
            self.gating_biases = nn.Parameter(
                torch.randn(output_dim) * 1 / math.sqrt(output_dim))
            self.bn1 = None
...
class NetVLADLoupe_GEM(nn.Module):
    def __init__(self, feature_size, max_samples, cluster_size, output_dim,
                 gating=True, add_batch_norm=True, is_training=True):
        super(NetVLADLoupe_GEM, self).__init__()
        self.feature_size = feature_size
        self.max_samples = max_samples
        self.output_dim = output_dim
        self.is_training = is_training
