    ...
    def collate_fn(data_list):
        ...
        if mink_quantization_size is None:
            ...
            if version == 'MinkLoc3D':
                ...
                plnts = []
                for e in batch:
                    plnts.append(torch.tensor(e.numpy(), dtype=torch.float))
                    # visual_pc(e)
                plnts = torch.stack(plnts, dim=0)
                ...
                for e in batch:
                    ...
                    plnts.append(torch.tensor(e.numpy()[:, 0:3], dtype=torch.float))
                    # visual_pc(e)
